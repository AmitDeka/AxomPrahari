...
export const citizenRequestOtp = async (req, res) => {
  try {
    ...
    res.status(200).json({ status: 'success', message: 'OTP sent successfully via WhatsApp' });
  } catch (error) {
...
export const citizenCompleteProfile = async (req, res) => {
  try {
    ...
    const token = jwt.sign(
      { id: updatedUser.id, role: updatedUser.role, status: updatedUser.profile_status },
      process.env.JWT_SECRET,
      { expiresIn: '7d' }
    );
...
export const adminLogout = async (req, res) => {
  try {
    ...
    res.status(200).json({
      status: 'success',
      message: 'Admin logged out securely. Token has been permanently invalidated.'
    });
  } catch (error) {
